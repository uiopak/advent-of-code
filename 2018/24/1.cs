    ...
    public class Program
    {
        ...
        public static void Main()
        {
            ...
            while (infectionGroups.Count > 0 && immuneGroups.Count > 0)
            {
                ...
                groupUnion = groupUnion.OrderBy(c => (c.Units * c.Damage)).ThenBy(c => c.Initiative).Reverse().ToList();
                foreach (var @group in groupUnion)
                {
                    if (@group.Gt == groupType.Immune)
                    {
                        var tmp = infectionGroups.Where(c => c.target == false)
                            .OrderByDescending(c => @group.TheoreticalDamage(c)).ThenByDescending(c => c.Units * c.Damage)
